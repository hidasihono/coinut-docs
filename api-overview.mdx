...
<Warning>
  Connectivity to these hosts may be gated by partner IP allowlisting. If you receive `403` with
  `error code: 1010`, the request was blocked before application-layer auth or validation ran.
</Warning>
...
<Note>
  The current OpenAPI file still lists `api_secret` as a security scheme. The narrative integration
  contract uses the secret as the HMAC signing key, not as a request header. Keep your implementation
  aligned with the signed-header flow shown here unless Coinut gives you an environment-specific override.
</Note>
...
<Note>
  The latest contract documents `200` and `422` at the API layer. Live testing on June 16, 2026 also
  observed infrastructure-level `403 error code: 1010` for signed requests from a non-allowlisted IP,
  and an unauthenticated `/health` response that returned HTTP `200` with an `UNAUTHORISED` body.
  Handle infrastructure and application responses separately in your client.
</Note>
