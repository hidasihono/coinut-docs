...
Offramp lets your customers convert cryptocurrency (USDT) into fiat currency deposited to their bank account. The flow works like this:
...
<Steps>
  ...
  <Step title="Fiat Arrives">
    ...
    | EUR | 1 business day (SEPA) |
    | GBP | 1 business day (Faster Payments) |
    | CAD | 1 business day |
    | AUD | 1 business day |
  </Step>
</Steps>
...
<Note>
  Always include `accountName` when available. Some banks require an exact name match for compliance.
</Note>
...
<Warning>
  Always verify the deposit address network matches the network the customer will send on. Sending USDT on the wrong network (e.g., sending TRC-20 to an ERC-20 address) will result in lost funds.
</Warning>
