...
<Tip>
  Use High (`H`) error correction level if you plan to overlay a logo or icon in the center of the QR code. This uses more data modules but allows up to 30% of the code to be damaged or covered while remaining scannable.
</Tip>

<Warning>
  Always serve QR code generation endpoints over HTTPS. Never generate QR codes client-side with sensitive payment details exposed in JavaScript.
</Warning>
