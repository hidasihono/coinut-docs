...
<CardGroup cols={3}>
  <Card title="Onramp" icon="arrow-up-right-from-square">
    Convert fiat to crypto. Your customers pay with bank transfers or cards, receive cryptocurrency directly to their wallets.
  </Card>
  <Card title="Offramp" icon="arrow-down-to-bracket">
    Convert crypto to fiat. Your customers send cryptocurrency, receive fiat currency via bank transfer.
  </Card>
  <Card title="Virtual Accounts" icon="building-columns">
    Receive fiat payments through dedicated Virtual Account numbers tied to individual customers.
  </Card>
</CardGroup>
...
```
┌─────────────────────┐      ┌──────────────────────────┐      ┌─────────────────────────┐
│     Partner App     │ ───▶ │   Coinut Partner Ramp    │ ───▶ │    Banking Network      │
│     (Your App)      │ ◀─── │           API            │ ◀─── │    + Blockchain         │
└─────────────────────┘      └──────────────────────────┘      └─────────────────────────┘
```
...
- **Multi-currency Support** — Fiat: USD, EUR, GBP, AUD, CAD, SGD. Crypto: USDT, USDC, and more
- **Global Compliance** — Licensed across 6 jurisdictions, automated KYC/AML
...
| Model | Use it when | Primary APIs |
| --- | --- | --- |
| **Autoramp** | You want Coinut to orchestrate the conversion route after setup | `/autoramp-flow/*`, `/virtual-account/*`, `/deposit-address/*` |
| **Omnibus** | You manage the customer experience and call each API directly | `/customer/*`, `/bank-account/*`, `/crypto-address/*`, `/deposit/*`, `/trade/*` |
