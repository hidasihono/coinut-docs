...
Generate scannable QR codes for two distinct payment types: **PayID** (AUD virtual account deposits) and **crypto address URIs** (onramp/offramp wallet addresses). A single scan populates all payment fields in the customer's banking or wallet app, eliminating transcription errors.

## Example Flow

<Steps>
  <Step title="Get a payment target">
    Partner already has a virtual account `payId` (fiat) or a verified deposit/crypto address (crypto) for the customer.
  </Step>
  <Step title="Generate the QR code">
    Partner encodes the `payId` or address into a QR code using the code samples below.
  </Step>
  <Step title="Customer scans">
    Customer scans the QR code in their banking or wallet app, which auto-populates the payment fields.
  </Step>
  <Step title="Coinut detects the deposit">
    Coinut detects the incoming payment and, if the target is linked to an autoramp flow, converts it automatically.
  </Step>
</Steps>
...
<Steps>
  <Step title="Prerequisite: get a payId">
    Create a virtual account via [Virtual Accounts](virtual-accounts-guide). The create response only returns the new VA's `id` — immediately call [`GET /virtual-account/detail`](api/virtual-account-detail) <Icon icon="link" size={14} /> to read `payId`.
  </Step>
  <Step title="Encode the format">
    PayID QR codes follow the format `payid:{payIdAddress}` — e.g. `payid:john.doe@example.com`. When scanned with an Australian banking app, the PayID field is auto-populated and the customer only needs to enter the amount and confirm.
  </Step>
</Steps>
...
<Note>
  A virtual account's deposits only auto-convert to crypto if it's linked to an onramp flow. If you only need to receive and reconcile fiat, no autoramp flow is required — see [Autoramp Flows](autoramp-flows) and [Onramp Integration](onramp-integration).
</Note>
...
The `address` you QR-encode must come from a Coinut deposit address record — **not** an arbitrary address you generate yourself. See [Crypto Addresses](crypto-addresses#deposit-addresses-offramp-entry) for how it's created and approved; it is **not** available synchronously on creation.
...
- [Virtual Accounts](virtual-accounts-guide) — creating a VA and fetching its `payId`
- [Crypto Addresses](crypto-addresses) — deposit address creation and approval
- [Autoramp Flows](autoramp-flows) — linking a VA or deposit address to automatic conversion
