...
| --- | --- | --- |
| `X-COINUT-PARTNER-APIKEY` | Yes | Your API key |
| `X-COINUT-PARTNER-NONCE` | Yes | Unique nonce timestamp in microseconds or UUID |
| `X-COINUT-PARTNER-SIGNATURE` | Yes | HMAC-SHA256 hex-encoded signature |
...
async function signAndRequest() {
  const method = 'POST';
  const host = 'partner-ramp-api.coinut.com'; // or partner-sandbox.coinut.net
...
public class CoinutSignatureExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String method = "POST";
        String host = "partner-ramp-api.coinut.com";
...
def main():
    method = "POST"
    host = "partner-ramp-api.coinut.com"  # or partner-sandbox.coinut.net
...
```bash
curl https://partner-sandbox.coinut.net/balance \
...
| --- | --- | --- |
| `401` | Invalid signature or unknown API key | Verify your API Secret and signature generation logic |
| `422` | Invalid parameters or failed operation | Check request body and query parameters |
| `409` | Nonce already used (if applicable) | Generate a fresh nonce for every request |
...
```bash
curl https://partner-sandbox.coinut.net/ip-white-list \
  -H "X-API-Key: $API_KEY" \
...
```bash
curl -X POST https://partner-sandbox.coinut.net/ip-white-list \
