...
Every request to the Coinut Partner Ramp API must be cryptographically signed using **HMAC-SHA256**. The server verifies the signature against your **API Secret** stored on the Coinut side. Requests with a missing, malformed, or invalid signature (or an unrecognized API key) are rejected with `401 Unauthorized`.
...
The following shows a full [`GET /balance`](api/common-balance) <Icon icon="link" size={14} /> request with all headers and a sample response.
...
| --- | --- | --- |
| `401` | Missing header, unrecognized API key, expired/incorrectly-formatted timestamp, or signature mismatch | Rebuild the canonical string exactly as shown above and re-check each header value |
| `422` | Request was authenticated but the body/query failed validation, or a downstream operation failed | Compare the request against the endpoint's schema in the API Reference |
...
| --- | --- | --- |
| `ipWhiteList` | Yes | Array of IP addresses, **maximum 6 entries**. Empty array `[]` means **no restriction**. |

<Note>
  Submitting more than 6 IP addresses is rejected with a `422` error (`Only 6 IP addresses are allowed`). If you operate more than 6 egress IPs, front them with a NAT gateway or reverse proxy and whitelist that single address instead.
</Note>
...
Each partner has exactly **one** active API key/secret pair — there is no overlap window. Issuing a new pair immediately invalidates the old one, so plan rotation as a cutover, not a gradual migration:

1. Generate a new key pair from the partner dashboard
2. Fully test the new credentials against sandbox with a build that isn't serving production traffic yet
3. Schedule a short maintenance window and deploy the new key/secret to all production instances at once
4. Confirm production traffic is signing successfully with the new pair

<Warning>
  Because only one key pair can be active at a time, any request signed with the old secret will start failing with `401 Unauthorized` the moment the new pair is saved — including in-flight requests from instances that haven't picked up the new secret yet. Roll out the new secret to every instance as close to simultaneously as your deployment tooling allows.
</Warning>
